...
class PlinkoEngine {
  ...
  /**
   * Whether the cheat key (]) is currently held down.
   * Applies a gentle outward nudge to all active balls each physics tick.
   */
  private isCheatKeyHeld = false;
  private cheatKeyDownHandler = (e: KeyboardEvent) => {
    if (e.key === ']') this.isCheatKeyHeld = true;
  };
  private cheatKeyUpHandler = (e: KeyboardEvent) => {
    if (e.key === ']') this.isCheatKeyHeld = false;
  };
  ...
  start() {
    ...
    window.addEventListener('keydown', this.cheatKeyDownHandler);
    window.addEventListener('keyup', this.cheatKeyUpHandler);

    // Each physics tick: if ] is held, nudge every ball gently outward from centre.
    Matter.Events.on(this.engine, 'beforeUpdate', () => {
      if (!this.isCheatKeyHeld) return;
      const centerX = this.canvas.width / 2;
      // Force is tiny — enough to drift over many rows, not enough to look forced.
      const NUDGE = 0.000035;
      Matter.Composite.allBodies(this.engine.world).forEach((body) => {
        if (body.collisionFilter.category !== PlinkoEngine.BALL_CATEGORY) return;
        const direction = body.position.x >= centerX ? 1 : -1;
        Matter.Body.applyForce(body, body.position, { x: direction * NUDGE, y: 0 });
      });
    });
  }
  ...
  stop() {
    ...
    window.removeEventListener('keydown', this.cheatKeyDownHandler);
    window.removeEventListener('keyup', this.cheatKeyUpHandler);
  }
